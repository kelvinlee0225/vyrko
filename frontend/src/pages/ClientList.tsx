...
const columns = 'grid-cols-[1fr_140px_140px_1fr_110px]'
...
export function ClientList() {
  ...
  const resultados = useMemo(() => {
    ...
    return clientes.filter((c) =>
      [c.nombre, c.cedulaRnc, c.telefono, c.correo].some((field) => field.toLowerCase().includes(q)),
    )
  }, [query, clientes])
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="overflow-x-auto rounded-lg border border-line bg-surface">
        <div className={`grid ${columns} min-w-[820px] gap-4 border-b border-line bg-surface-alt px-6 py-2 text-[11px] font-semibold uppercase tracking-[0.04em] text-muted`}>
          <span>Nombre</span>
          <span>Cédula / RNC</span>
          <span>Teléfono</span>
          <span>Correo</span>
          <span className="text-right">Vehículos</span>
        </div>
        <div className="flex min-w-[820px] flex-col">
          ...
          {resultados.map((c) => (
            ...
            >
              ...
              <span className="overflow-hidden text-ellipsis whitespace-nowrap text-[12.5px] text-muted">{c.correo}</span>
              <span className="text-right text-[12.5px] text-muted">
                {c.vehiculos.length === 0 ? '—' : c.vehiculos.length}
              </span>
            </Link>
          ))}
        </div>
